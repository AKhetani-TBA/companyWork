...
namespace ExcelAddIn5
{
    class ConnectionManager : IDisposable
    {
        ...
        public void LoadCustomAddIns(bool Status, bool callValue, string AddinName)
        {
        ...
        public void LoadCustomAddIn(string subKeyName, string selectedID)
        {
            try
            {
                object addinRef = subKeyName;
                ...
                if (G_Addi.Connect == true)
                {
                    ...
                    G_object.Load(false, AuthenticationManager.Instance.userEmailID, selectedID);
                    G_object.Load(true, AuthenticationManager.Instance.userEmailID, selectedID);
        ...
        public void LoadImage(string subKeyName, string selectedID, COMAddIn G_Addi)
        {
            try
            {
                object addinRef = subKeyName;
                //Microsoft.Office.Core.COMAddIn G_Addi = x.Application.COMAddIns.Item(ref addinRef);
                if (G_Addi.Connect == true)
                {
                    var G_object = G_Addi.Object; // G_Addi.Object;
                    ...
                    G_object.Load(false, "akhetani@thebeastapps.com", selectedID);
                    G_object.Load(true, "akhetani@thebeastapps.com", selectedID);
        ...
        public void OnSignalrConnectionStatusChanged(SignalrConnectionStatusChanged evt)
        {
            if (evt._isReconnect == false)
                ConnectionManager.Instance.LoadCustomAddIns(Convert.ToBoolean(SignalRConnectionStatus.SIGNALR_CONNECTED), false,"");
            else
            {
