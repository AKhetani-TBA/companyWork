...
public class DeductionEarning
{
	...
	public int SaveDeduction_Slab(VCM.EMS.Base.DeductionEarning objDeduction_Slab)
	{
		...
		try
		{
			...
			cmd = dbHelper.GetStoredProcCommand("DeductionEarning_Save");
            ...
            dbHelper.AddInParameter(cmd, "@slabType", DbType.String, objDeduction_Slab.SlabType );
            dbHelper.AddInParameter(cmd, "@slabOrder", DbType.Int32, objDeduction_Slab.SlabOrder);
	...
	public int DeleteDeduction_Slab(System.Int64 slabId)
	{
		...
		{
			dbHelper = DatabaseFactory.CreateDatabase();
			cmd = dbHelper.GetStoredProcCommand("DeductionEarning_Delete");
	...
	public List<VCM.EMS.Base.DeductionEarning> GetAll(Boolean  isActive)
	{
		...
		{
			dbHelper = DatabaseFactory.CreateDatabase();
            cmd = dbHelper.GetStoredProcCommand("DeductionEarning_Select");
			dbHelper.AddInParameter(cmd,"@IsActive",DbType.Boolean,isActive);
			IDataReader objReader = dbHelper.ExecuteReader(cmd);
			List<VCM.EMS.Base.DeductionEarning> lstReturn = new List<VCM.EMS.Base.DeductionEarning>();
			while (objReader.Read())
			{
				VCM.EMS.Base.DeductionEarning objDeduction_Slab = new VCM.EMS.Base.DeductionEarning();
    ...
    public int GetCountSlabChilds( System.Int64 slabId)
	{
		...
		{
			dbHelper = DatabaseFactory.CreateDatabase();
            cmd = dbHelper.GetStoredProcCommand("DeductionEarning_CountChildRecords");
	...
	public VCM.EMS.Base.DeductionEarning GetDeduction_SlabByID(string order,string fieldName,System.Int64 slabId)
	{
		...
		try
		{
			...
			dbHelper.AddInParameter(cmd,"@slabId",DbType.Int64,slabId);
            dbHelper.AddInParameter(cmd, "@slabType", DbType.Int64, -1);
			IDataReader objReader = dbHelper.ExecuteReader(cmd);
			VCM.EMS.Base.DeductionEarning objDeduction_Slab = new VCM.EMS.Base.DeductionEarning();
			if (objReader.Read())
			{
    ...
    public System .Data .DataSet  GetAllDs(string order, string fieldName, System.Int64 slabId,int slabType)
    {
        ...
        try
        {
            ...
            dbHelper.AddInParameter(cmd, "@fieldName", DbType.String, fieldName);
            dbHelper.AddInParameter(cmd, "@slabId", DbType.Int64, slabId);
            dbHelper.AddInParameter(cmd, "@slabType", DbType.Int64, slabType);
	...
	protected VCM.EMS.Base.DeductionEarning MapFrom(IDataReader objReader)
	{
		VCM.EMS.Base.DeductionEarning objDeduction_Slab = new VCM.EMS.Base.DeductionEarning();
        ...
        if (!Convert.IsDBNull(objReader["slabType"])) { objDeduction_Slab.SlabType  = Convert.ToString(objReader["slabType"], CultureInfo.InvariantCulture); }
        if (!Convert.IsDBNull(objReader["slabOrder"])) { objDeduction_Slab.SlabOrder = Convert.ToInt32(objReader["slabOrder"], CultureInfo.InvariantCulture); }
